...
public sealed class WindowPlacementTool(ElementRegistry refs, PendingInvokeTracker pending, SessionManager? sessions = null) : ToolBase
{
    ...
    public override Task<McpToolResult> ExecuteAsync(JsonElement? arguments)
    {
        ...
        if (reference == null && handle == null) return Task.FromResult(ErrorResult("ref or handle required"));
        if (handle != null && sessions == null) return Task.FromResult(ErrorResult("Window-handle placement requires a session manager."));
        if (reference != null && handle != null) refs.ValidateReference(reference, handle);
        ...
        if (reference != null)
        {
            ...
            if (element.Properties.ControlType.ValueOrDefault != ControlType.Window || native == 0 || native != target.Hwnd)
        ...
        {
            OperationContext.Check();
            if (reference != null) refs.ValidateReference(reference);
            target.EnsureAlive();
